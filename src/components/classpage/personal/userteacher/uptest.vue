<template>
  <div id="app">
    <div class="contain">
      ...
      <b>选择目录：</b>
      <i-select style="width:200px">
        <i-option v-for="item in courseList" :value="item.value" :key="item.value">{{ item.label }}</i-option>
      </i-select>
      <br>
      <br>
      <b>选择题：</b>
      ...
      <div class="divone" v-for="index in customized_descsone" :key="index">
        ...
        <div class="Answer">
          <div class="ans">
            <input type="checkbox" v-model="customizedone2" label style="width:20px;height:20px;">正确
          </div>
          <div class="ans">
            <input type="checkbox" v-model="customizedone3" label style="width:20px;height:20px;">错误
          </div>
        </div>
        <br>
        <b>该题的正确答案是：</b>
        <div class="Answer">
          <div class="ans">
            <input type="checkbox" v-model="customizedone4" label style="width:20px;height:20px;">正确
          </div>
          <div class="ans">
            <input type="checkbox" v-model="customizedone5" label style="width:20px;height:20px;">错误
          </div>
        </div>
      </div>
      <br>
    ...
    <br>
    <div class="right">
      <i-button style="color:white;background-color:#3d6ea7" @click="success(false)">上传测试</i-button>
    </div>
  </div>
</template>
